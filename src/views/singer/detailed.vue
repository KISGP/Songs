<template>
	<el-scrollbar>
		<div class="back">
			<div class="data">
				<div class="head">
					...
					<div class="message">
						...
						<div class="signature">
							<span>个性介绍：{{ artistData?.info.signature || "无" }}</span>
						</div>
						<div>
				...
				<div class="content">
					<el-tabs v-model="activePage" :stretch="true" @tab-change="pageChange">
			...
			<div class="other">
				<div>
					<span @click="artistIntroductionVisible">
						歌手信息
				...
				<div>
					<span @click="changeSimilarArtistVisible">
						相似歌手
						<el-icon><ArrowRightBold /></el-icon>
					</span>
				</div>
	...
	<el-dialog v-model="artistIntroduction" title="歌手信息" width="50%" center>
		<artistIntroductionDetailed :introduction="artistData?.introduction" />
	</el-dialog>

	<el-dialog v-model="similarArtistVisible" title="相似歌手" width="50%" center>
...
import { getDetailedArtist, getArtistHotSongs, getSimilarArtists } from "service/api/api";
...
const pageChange = (newPage: TabPaneName) => {};

const artistIntroduction = ref<boolean>(false);
const artistIntroductionVisible = () => {
	artistIntroduction.value = !artistIntroduction.value;
};

const similarArtistVisible = ref<boolean>(false);
const similarArtists = ref<Array<artistBriefType>>();
const changeSimilarArtistVisible = () => {
	similarArtistVisible.value = !similarArtistVisible.value;
};
</script>
