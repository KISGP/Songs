<template>
	<div class="min">
		<el-row :gutter="20">
					...
					<div class="audioBox" ref="audioBoxRef">
						...
						<div class="barBox">
							<div class="messageBox" ref="messageBoxRef">
								<!-- 歌名 -->
								<span> {{ SongStore.song?.name || "歌名" }} </span>
								<!-- 时间 -->
			...
			<el-col :span="6">
				<div class="right">
					<div :class="{ liked: true, 'liked-true': SongStore.song?.isLiked }">
						<el-icon
							size="25"
							:title="SongStore.song?.isLiked ? '移出我喜欢的音乐' : '添加我喜欢的音乐'"
							@click="SongStore.likeSong(!SongStore.song?.isLiked!)"
						>
							<svg-icon name="like" />
						</el-icon>
					</div>
	...
	<el-drawer v-model="list.visible" :with-header="false">
		<el-tabs v-model="activeList" class="demo-tabs">
			<el-tab-pane label="播放列表" name="播放列表">
				<play-list :songs="DataStore.playList" />
			</el-tab-pane>
			<el-tab-pane label="历史播放" name="历史播放">
				<play-list :songs="DataStore.historyList" />
			</el-tab-pane>
		</el-tabs>
	</el-drawer>
</template>
...
// 下载
const download = () => {
	SongStore.song?.url && downloadFile(SongStore.song.url, SongStore.song.name);
};
</script>
